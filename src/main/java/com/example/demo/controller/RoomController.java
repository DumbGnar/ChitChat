...
public class RoomController {
    ...
    public Room getRoom(@PathVariable int rid) {
        return mongoTemplate.findOne(query(where("_id").is(rid)), Room.class);
    }

    @RequestMapping("/user/getroom/{uid}")
    public List<Room> userGetRoom(@PathVariable int uid) {
        List<Room> roomList = new ArrayList<>();
        List<RoomSetting> roomSettings = mongoTemplate.find(query(where("myId").is(uid)), RoomSetting.class);
    ...
    private void addroomuserProduce(int rid, int uid) throws Exception {
        ...
        Update update = new Update();
        update.set("Allusers", added.addoneuser(uid));
        this.mongoTemplate.updateFirst(query, update, Room.class, this.collection_name);
    }
